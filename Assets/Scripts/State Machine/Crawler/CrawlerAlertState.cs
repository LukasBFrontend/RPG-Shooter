...
public class CrawlerAlertState : NPCBaseState
{
    const float PATH_CHECK_DISTANCE = 2f;
    private static WaitForSeconds _waitForSeconds0_1 = new(0.1f);
    Attack _attack;
    public CrawlerAlertState(NPCStateMachine currentContext, NPCStateFactory npcStateFactory) : base(currentContext, npcStateFactory)
    {
    ...
    Vector2 RandomClearDirection()
    {
        ...
        {
            float _random = Random.Range(0, Mathf.PI);

            Vector2 _dir = new(Mathf.Cos(_random), Mathf.Sin(_random));
